...
if [ -z "$CHECKOUT" ] || [ ! -d "$CHECKOUT/packages" ]; then
  if [ -x "./node_modules/.bin/tsc" ] || [ -f "./node_modules/.bin/tsc.cmd" ]; then
    echo "=== No DSH checkout; building with local devDependencies ==="
    ./node_modules/.bin/tsc -p tsconfig.json
    npm run build:client
    exit 0
  fi
  echo "build: cannot locate the dsh checkout and local tsc is unavailable" >&2
  echo "hint: run npm install, or set DSH_CHECKOUT to a dsh source checkout" >&2
  exit 1
fi
...
node -e "
  ...
  for (const name of ['cordis', 'cosmokit', 'schemastery']) {
    fs.rmSync('node_modules/' + name, { recursive: true, force: true });
  }
