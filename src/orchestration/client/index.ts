 ...
 *
 * Registers three slot occupants:
 * - `conversation.session.header.utilities`: the "自动续跑" button and the
 *   read-only "复制日志地址" button.
...
import { CopyLogButtonFor } from './copy-log-button.js'
	...
	ctx.effect(
		() =>
			ctx.slots.inject('conversation.session.header.utilities', () =>
				ctx.slots.register({
					name: 'conversation.session.header.utilities',
					id: 'session-resume-copy-log',
					order: 20,
					label: () => '复制地址',
				}, CopyLogButtonFor(ctx)),
			),
		'session-resume: header copy log',
	)
